class ChallengesController < ApplicationController
  ...
  before_action :fetch_challenge,  only: %i(show edit update destroy resolve find_mistake)
  ...
  def resolve
    if check_answer!(params[:raw_text])
  ...
  def find_mistake
    check_answer!(params[:raw_text])

    result = {}
    result[:correct] = @answer.correct?
    result[:mistake] = @answer.mistake

    if params[:require_cloze_text]
      result[:cloze_text] = @answer.fetch_cloze_text!
    end

    @answer.detect_partial_answer! if params[:require_partial_answer]
  ...
  def check_answer!(raw_text)
    answer = raw_text || ''
    @answer = @challenge.build_answer(answer)
    @answer.check!
  end
